...
### 规则3：单文件行数限制
单个源文件的有效代码行数不得超过 **600 行**（不含注释和空行）：
- 当文件接近或超过 600 行时，必须进行拆分或重构
- 每个文件应有**单一的、清晰的职责**，边界明确
- 拆分原则：
  - 按功能域拆分（如数据处理、UI 渲染、网络请求分离到不同文件）
  - 按抽象层级拆分（如接口定义、实现细节、工具函数分离）
  - 提取公共逻辑到共享模块，避免重复代码膨胀文件体积
- 目标：保持文件可快速浏览、可独立测试、可低认知负荷维护
...
- **Rust**：运行 `cargo check --bin ai-coding-desktop` 和 `cargo check --lib -p ai-coding-desktop`，确保 0 warnings
- **TypeScript**：运行 `npx tsc --noEmit -p tsconfig.check.json`，确保 0 errors
- 对于确实需要保留的代码（如预留的公共 API、未来的扩展点），使用 `#[allow(...)]` 或 `// @ts-ignore` 等显式抑制，并在注释中说明理由
- 禁止通过批量添加 `#[allow(dead_code)]` 等方式掩盖真正的代码质量问题
...
### 规则10：规则持久化
以上十条规则（规则1-10）在变更 AGENTS.md 时必须保留，不允许被覆盖、删除或修改。任何对 AGENTS.md 的更新都应在保留这些规则的前提下进行追加或调整其他内容。
