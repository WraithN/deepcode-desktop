...
pub async fn run(args: GwdArgs) -> Result<(), anyhow::Error> {
                ...
                for admin_port in [2346u16, 2347, 2348, 2349, 2350] {
                        ...
                        .await
                    {
                        if resp.status().is_success() {
                            ...
                            println!(
                                "    OpenAI: http://127.0.0.1:{}/v1/chat/completions",
                                api_port
                            );
                                ...
                                .await
                            {
                                if agents_resp.status().is_success() {
                                    let agents: Vec<serde_json::Value> =
                                        agents_resp.json().await.unwrap_or_default();
                                    if agents.is_empty() {
                                        println!("  Agents: none");
                                    } else {
                                        ...
                                        for agent in agents {
                                            ...
                                            let name = agent
                                                ...
                                                .and_then(|v| v.as_str())
                                                .unwrap_or(id);
                                            let status = gatewayd_support::format_agent_status(
                                                agent.get("status"),
                                            );
    ...
    if let GatewaydCommands::Logs { limit, session_id } = &cmd {
        ...
        let rows: Vec<_> = match &session_id {
            Some(sid) => stmt
                .query_map([sid.as_str()], gatewayd_support::row_mapper)?
                .collect::<Result<Vec<_>, _>>()?,
            None => stmt
                .query_map([], gatewayd_support::row_mapper)?
                .collect::<Result<Vec<_>, _>>()?,
        };
    ...
    if let GatewaydCommands::Session { session_id } = &cmd {
        ...
        let audit_rows = if let Some(ref meta) = session_meta {
            ...
            let mut stmt = conn.prepare(sql)?;
            let rows: Vec<_> = stmt
                .query_map([meta.0.as_str()], gatewayd_support::session_row_mapper)?
                .collect::<Result<Vec<_>, _>>()?;
            rows
        } else {
            ...
            let rows: Vec<_> = stmt
                .query_map(
                    [format!("{}%", target_id)],
                    gatewayd_support::session_row_mapper,
                )?
                .collect::<Result<Vec<_>, _>>()?;
            rows
        };
    ...
    if let GatewaydCommands::Request { request_id } = &cmd {
        ...
        let mut stmt = conn.prepare(sql)?;
        let rows: Vec<_> = stmt
            .query_map([target_id.as_str()], gatewayd_support::session_row_mapper)?
            .collect::<Result<Vec<_>, _>>()?;
    ...
    } = &cmd
    {
        ...
        if *json {
            ...
            println!(
                "│ {:<20} │ {:>7} │ {:<9} │",
                "Upstream Tokens", prompt, source
            );
            println!(
                "│ {:<20} │ {:>7} │ {:<9} │",
                "Downstream Tokens", completion, source
            );
